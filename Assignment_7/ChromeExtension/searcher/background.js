...
//                 // Ensure that the highlight box is properly positioned and resizes with the element
//                 Object.assign(highlight.style, {
//                   position: "absolute",
//                   top: `${window.scrollY + rect.top}px`,
//                   left: `${window.scrollX + rect.left}px`,
        ...
        if (tabId === newTab.id && changeInfo.status === "complete") {
          ...
          chrome.scripting.executeScript({
            ...
            func: async (xpathStr) => {
              ...
              try {
                console.log("[Injected Script 1] Waiting for element...");
                ...
                console.log("[Injected Script 1] Highlight added to element.");
              } catch (err) {
                console.error("[Injected Script 1] Error:", err);
                // Fallback: inject the second script if the first one fails
                chrome.scripting.executeScript({
                  target: { tabId: newTab.id },
                  func: async (xpathStr) => {
                    try {
                      console.log("[Injected Script 2] Fallback attempt for element...");

                      const element = await waitForElement(xpathStr);
                      element.scrollIntoView({ behavior: "smooth", block: "center" });

                      // Ensure visibility and other adjustments like in Script 1...
                      const computedStyle = window.getComputedStyle(element);
                      if (computedStyle.display === "none") element.style.display = "block";
                      if (computedStyle.visibility === "hidden") element.style.visibility = "visible";
                      if (computedStyle.opacity === "0") element.style.opacity = "1";

                      // Highlight box and other modifications
                      const highlight = document.createElement("div");
                      const rect = element.getBoundingClientRect();
                      ...
                      Object.assign(highlight.style, {
                        ...
                        width: `${rect.width}px`,
                        height: `${rect.height}px`,
                        border: "5px solid blue", // Different color to differentiate the fallback
                        boxSizing: "border-box",
                        zIndex: "9999",
                        pointerEvents: "none",
                        animation: "flash-border 1s infinite",
                      ...
                      console.log("[Injected Script 2] Highlight added to element.");
                    } catch (err) {
                      console.error("[Injected Script 2] Error:", err);
                      alert("Both attempts to highlight the element failed.");
                    }
                  },
                  args: [xpath]
                });
